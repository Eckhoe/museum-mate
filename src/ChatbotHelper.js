...
export function LevenshteinDistance(input, candidate){
    ...
    let cost, cosIns, cosDel, cosSub; // Cost and cost of operations
    ...
    for(let j = 1; j <= canLen; j++){
        for(let i = 1; i <= inLen; i++){
            ...
            cosIns = distMat[j - 1][i] + 1; // Cost of Deletion
            cosDel = distMat[j][i - 1] + 1; // Cost of Insertion
            cosSub = distMat[j - 1][i - 1] + cost;  // Cost of Substitution
            distMat[j][i] = Math.min(cosIns, cosDel, cosSub);
        }
    }
